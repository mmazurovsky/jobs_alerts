...
import uuid
from src.data.data import JobSearchData, NewJobSearch, JobType, RemoteType, TimePeriod
...
class JobSearchManager:
    ...
    def __init__(self):
        ...
        self._job_searches: Dict[int, List[JobSearchData]] = {}
    
    def add_job_search(self, new_search: NewJobSearch) -> JobSearchData:
        """Add a new job search for a user."""
        if new_search.user_id not in self._job_searches:
            self._job_searches[new_search.user_id] = []
        
        job_search = JobSearchData(
            id=str(uuid.uuid4()),
    ...
    def get_user_job_searches(self, user_id: int) -> List[JobSearchData]:
        """Get all job searches for a user."""
        return self._job_searches.get(user_id, [])
    
    def get_job_search(self, user_id: int, search_id: str) -> Optional[JobSearchData]:
        """Get a specific job search for a user."""
        if user_id not in self._job_searches:
            return None
        
        for search in self._job_searches[user_id]:
            if search.id == search_id:
                return search
        return None
